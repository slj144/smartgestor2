  ...
  export class iTools {
    ...
    public static FieldValue = new class{
      ...
      public replace(propertyName: string, prefixCode: boolean = false, trim: boolean = false, batchRef: number = -1){
        if(batchRef != -1){
          return `$replace(${propertyName},${batchRef},${trim},${prefixCode})`;
        }else{
          return `$replace(${propertyName},${trim},${prefixCode})`;
        }
      }
