...
export class ProvidersRegisterComponent implements OnInit {
  ...
  public translate = ProvidersTranslate.get()['modal']['action']['register'];
  ...
  public onSubmit() {
    ...
    const formData = this.formProvider.value;
    const source = this.settings.data;

    const data: IRegistersProvider = {
      _id: source._id,
      code: source.code,
    ...
    this.providersService.registerProvider(data).then(() => {
  ...
  private formSettings(data: any = {}) {
    ...
    this.formProvider = this.formBuilder.group({
      ...
      contacts: this.formBuilder.group({
        ...
        phone: [(data.contacts && data.contacts.phone ? data.contacts.phone : ''), (this.isBrazil ? [Validators.minLength(14), Validators.maxLength(16)] : [])]
      }),
