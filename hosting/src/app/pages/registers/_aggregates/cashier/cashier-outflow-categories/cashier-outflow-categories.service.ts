...
// Translate
import { CashierOutflowCategoriesTranslate } from "./cashier-outflow-categories.translate";

// Interfaces
import { IBatch } from '@itools/interfaces/IBatch';
import { ICollection } from '@itools/interfaces/ICollection';
import { ICashierOutflowCategory } from '@shared/interfaces/ICashierOutflowCategory';
...
export class CashierOutflowCategoriesService {

  public translate = CashierOutflowCategoriesTranslate.get();
  ...
  public async registerCategory(data: ICashierOutflowCategory, batch?: IBatch): Promise<void> {
  ...
  public async deleteCategory(data: ICashierOutflowCategory, batch?: IBatch): Promise<void> {
  ...
  private async systemLogs(data: ICashierOutflowCategory, action: string, batch: IBatch, batchRef?: number) {
    ...
    settings.data[0].type = ESystemLogType.CashierOutflowCategories;
  ...
  private collRef(): ICollection {
    return this.iToolsService.database().collection('CashierOutflowCategories');
  }
