...
// Translate
import { CashierInflowCategoriesTranslate } from "./cashier-inflow-categories.translate";

// Interfaces
import { IBatch } from '@itools/interfaces/IBatch';
import { ICollection } from '@itools/interfaces/ICollection';
import { ICashierInflowCategory } from '@shared/interfaces/ICashierInflowCategory';
...
export class CashierInflowCategoriesService {

  public translate = CashierInflowCategoriesTranslate.get();
  ...
  public async registerCategory(data: ICashierInflowCategory, batch?: IBatch): Promise<void> {
  ...
  public async deleteCategory(data: ICashierInflowCategory, batch?: IBatch): Promise<void> {
  ...
  private async systemLogs(data: ICashierInflowCategory, action: string, batch: IBatch, batchRef?: number) {
    ...
    settings.data[0].type = ESystemLogType.CashierInflowCategories;
  ...
  private collRef(): ICollection {
    return this.iToolsService.database().collection('CashierInflowCategories');
  }
