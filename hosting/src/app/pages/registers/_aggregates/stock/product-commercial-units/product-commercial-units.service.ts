...
// Translate
import { ProductCommercialUnitTranslate } from "./product-commercial-units.translate";

// Interfaces
import { IBatch } from '@itools/interfaces/IBatch';
import { ICollection } from '@itools/interfaces/ICollection';
import { IStockProductCommercialUnit } from '@shared/interfaces/IStockProductType';
...
export class ProductCommercialUnitsService {

  public translate = ProductCommercialUnitTranslate.get();
  ...
  public getUnits(listenerId: string, listener: ((_: any)=>void)) {
  ...
  public async registerUnit(data: IStockProductCommercialUnit, batch?: IBatch): Promise<void> {
  ...
  public async deleteUnit(data: IStockProductCommercialUnit, batch?: IBatch): Promise<void> {
  ...
  private async systemLogs(data: IStockProductCommercialUnit, action: string, batch: IBatch, batchRef?: number) {
    ...
    settings.data[0].type = ESystemLogType.StockProductCommercialUnit;
  ...
  private collRef(): ICollection {
    return this.iToolsService.database().collection('StockProductUnitCommecial');
  }
