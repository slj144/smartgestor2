...
export class VehiclesService {
  ...
  private data: { [_id: string]: IRegistersVehicles } = {};
  ...
  public query(where?: query['where'], reset: boolean = true, flex: boolean = false, scrolling: boolean = false, strict: boolean = true, others?: {back?: boolean, orderBy?: any }) {
    ...
    return (new Promise<IRegistersVehicles[]>((resolve) => {
  ...
  public getVehicles(listenerId: string, listener: ((_: any)=>void)) {
  ...
  public async registerVehicle(data: IRegistersVehicles, batch?: IBatch) {
  ...
  public async deleteVehicle(data: IRegistersVehicles, batch?: IBatch) {
  ...
  public getVehiclesCount(listenerId: string, listener: ((_: any)=>void)) {
  ...
  private async systemLogs(data: IRegistersVehicles, action: string, batch: IBatch, batchRef?: number) {
  ...
  private collRef(settings?: query): ICollection {
    ...
    const collection = this.iToolsService.database().collection('RegistersVehicles');
  ...
  private async checkRelatedCollections(remoteData, newData, batch, batchRef){
    ...
    const serviceOrders = (await this.iToolsService.database().collection("ServiceOrders").where([
      { field: "vehicle.plate", operator: "=", value: remoteData.plate },
  ...
  private treatData(id: string, data?: IRegistersVehicles[], order: 1 | -1 = 1) {
